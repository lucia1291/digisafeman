(function(){
  ...
  const CATEGORY = (params.get('category') || 'MACCHINARI').toUpperCase(); // default
  const TOPIC = (params.get('topic') || 'TORNIO MANUALE').toUpperCase();
  const LEVEL_NUMBER = parseInt(params.get('level') || '1', 10);

  // ====== DOMANDE (catalogo) ======
  const QUESTIONS = QUESTIONS_BANK[TOPIC]?.[LEVEL_NUMBER] ?? [
    { q:'Domanda non definita per questo livello.', a:['OK'], correct:0 }
  ];
  ...
  btnRetry.addEventListener('click',()=>{
    ...
    // ripristina stato pulsanti nel caso precedente game-over cuori
  ...
  function endLevel(failedByHearts){
    ...
    // === LOGICA Riprova / Torna al menu ===
    if (failedByHearts || hearts <= 0) {
      btnRetry.classList.add('hidden');     // nascondi Riprova per evitare duplicato
      btnRetry.classList.remove('ghost');   // pulizia eventuale
      btnRetry.textContent = 'Riprova';     // reset testo per usi futuri
      // assicurati che il link "Torna al menu" sia visibile e focalizzato
      btnMenu.classList.remove('hidden');
      setTimeout(()=>btnMenu.focus(), 0);
    } else {
      // caso normale: Riprova visibile e "Torna al menu" resta com'è
